...
class JITDebugStateLog {
	...
	public:
		/**
		 * Logs the complete GBA state to the SD card buffer.
		 ...
		 */
		void Init();
		void LogState(const char* source, u32 executedPC, u32 nextPC, u32 ticks);
		void WriteToFile();
};
...
extern JITDebugStateLog jitDebugStateLog;
	...
	#define JIT_LOG_STATE_CPP(executedPC, nextPC, ticks)		jitDebugStateLog.LogState("[C++]", (executedPC), (nextPC), (ticks))
    #define JIT_LOG_STATE_JIT(executedPC, nextPC, ticks)		jitDebugStateLog.LogState("[JIT]", (executedPC), (nextPC), (ticks))
	#define JIT_LOG_STATE_WRITE_TO_FILE()						jitDebugStateLog.WriteToFile()
#else
	#define JIT_LOG_STATE_INIT()        						((void)0)
	#define JIT_LOG_STATE_CPP(executedPC, nextPC, ticks)        ((void)0)
    #define JIT_LOG_STATE_JIT(executedPC, nextPC, ticks)        ((void)0)
