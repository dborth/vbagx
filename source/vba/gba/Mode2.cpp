...
static inline void mode2RenderLine_Impl() {
  ...
  u32* l2 = line2; u32* l3 = line3;
  u32* lO = lineOBJ; u32* lM = lineMix;

  for(u32 x = 0; x < 240u; ++x) {
    u32 c2 = *l2++; u32 c3 = *l3++; u32 cO = *lO++;
    u32 color = backdrop;
    u8 top = 0x20;
    ...
    if((top & 0x10) && (color & 0x00010000)) {
      ...
      if(r_back < (u8)(back >> 24)) {
        if(r_back == li2) { back = c2; top2 = 0x04; }
    ...
    if(!(color & 0x00010000)) {
      if (EFFECT == 1) {
        if(top & BLDMOD) {
          ...
          if((top != 0x08) && (u8)(c3>>24) < (u8)(back >> 24)) { back = c3; top2 = 0x08; }
...
static inline void mode2RenderLineAll_Impl() {
  ...
  for(u32 x = 0; x < 240u; ++x) {
    ...
    if((mask & 4) && c2 < color) { color = c2; top = 0x04; }
