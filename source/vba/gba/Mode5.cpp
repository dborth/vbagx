...
static inline bool mode5CheckBlanking() {
  if(DISPCNT & 0x0080) {
    int x = 232;
    do {
...
template <int EFFECT>
static inline void mode5RenderLine_Impl() {
  if(mode5CheckBlanking()) return;
  u16 *palette = (u16 *)paletteRAM;

  if(layerEnable & 0x0400) {
    int changed = gfxBG2Changed;
    if(gfxLastVCOUNT > VCOUNT) changed = 3;
    gfxDrawRotScreen16Bit160(BG2CNT, BG2X_L, BG2X_H, BG2Y_L, BG2Y_H, BG2PA, BG2PB, BG2PC, BG2PD, gfxBG2X, gfxBG2Y, changed, line2);
  }
  ...
  for(int x = 0; x < 240; ++x) {
    ...
    if((top & 0x10) && (color & 0x00010000)) {
      ...
      if(line2[x] < back) { back = line2[x]; top2 = 0x04; }
...
template <int EFFECT>
static inline void mode5RenderLineNoWindow_Impl() {
  if(mode5CheckBlanking()) return;
  u16 *palette = (u16 *)paletteRAM;
...
template <int EFFECT>
static inline void mode5RenderLineAll_Impl() {
  if(mode5CheckBlanking()) return;
  u16 *palette = (u16 *)paletteRAM;
  ...
  for(int x = 0; x < 240; ++x) {
    ...
    if(color & 0x00010000) {
      ...
      if((mask & 4) && line2[x] < back) { back = line2[x]; top2 = 0x04; }
      ...
      if (EFFECT == 1) {
        if(top & BLDMOD) {
          ...
          if((mask & 4) && (top != 0x04) && (line2[x] < background)) { back = line2[x]; top2 = 0x04; }
...
void mode5RenderLine() {
  switch((BLDMOD >> 6) & 3) {
    case 0: mode5RenderLine_Impl<0>(); break;
    case 1: mode5RenderLine_Impl<1>(); break;
    case 2: mode5RenderLine_Impl<2>(); break;
    case 3: mode5RenderLine_Impl<3>(); break;
  }
}

void mode5RenderLineNoWindow() {
  switch((BLDMOD >> 6) & 3) {
    case 0: mode5RenderLineNoWindow_Impl<0>(); break;
    case 1: mode5RenderLineNoWindow_Impl<1>(); break;
    case 2: mode5RenderLineNoWindow_Impl<2>(); break;
    case 3: mode5RenderLineNoWindow_Impl<3>(); break;
  }
}

void mode5RenderLineAll() {
  switch((BLDMOD >> 6) & 3) {
    case 0: mode5RenderLineAll_Impl<0>(); break;
    case 1: mode5RenderLineAll_Impl<1>(); break;
    case 2: mode5RenderLineAll_Impl<2>(); break;
    case 3: mode5RenderLineAll_Impl<3>(); break;
  }
}
