...
void OgcEmulatorVideo::drawFps(const char* str, f32 x, f32 y) {
	if (!str) return;
 ...
 * console resolution or the active border changes.
 ***************************************************************************/
void OgcEmulatorVideo::recalculateScaling()
{
	...
	if (EmuSettings.videoAspectRatioCorrection == SCALING_PARTIAL_STRETCH)
	...
	square[3] = square[6]  =  xscale + EmuSettings.videoXshift;
	square[1] = square[4]  =  yscale - EmuSettings.videoYshift;
	...
	if (fixed) {
		...
		// 240p adjustment
		if (EmuSettings.videoMode == VIDEOMODE_240P || EmuSettings.videoMode == VIDEOMODE_EURGB_240P) vw *= 2;
	...
	float efbCenterX = vpX + vpW * ((320.0f + EmuSettings.videoXshift) / 640.0f);
	float efbCenterY = vpY + vpH * ((240.0f + EmuSettings.videoYshift) / 480.0f);
	...
	float vi_to_menu_x = (float)videoDriver->getScreenWidth() / (float)menu_vmode->viWidth;
	float vi_to_menu_y = (float)videoDriver->getScreenHeight() / (float)menu_vmode->viHeight;
 ...
 ****************************************************************************/
void OgcEmulatorVideo::presentFrame(int consoleWidth, int consoleHeight)
{
	u8* buffer = pix;
